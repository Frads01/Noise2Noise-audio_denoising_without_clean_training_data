...
# Principali fonti/riferimenti:
# https://github.com/schmiph2/pysepm


class AudioMetricException(Exception):
    """
    Eccezione personalizzata per gli errori relativi alle metriche audio.
    """
    pass
...
def resample(original, old_rate, new_rate):
    ...
    Returns:
        np.ndarray: Il segnale audio ricampionato.
    """
...
def extract_overlapped_windows(x, nperseg, noverlap, window=None):
    """
    Estrae finestre sovrapposte da un segnale.

    Args:
        x (np.ndarray): Il segnale di input.
        nperseg (int): La lunghezza di ciascuna finestra.
        noverlap (int): Il numero di campioni sovrapposti tra finestre adiacenti.
        window (np.ndarray, optional): La finestra da applicare a ciascun segmento. Se None, non viene applicata alcuna finestra.

    Returns:
        np.ndarray: Un array contenente le finestre sovrapposte.
    """
...
def find_loc_peaks(slope, energy):
    """
    Trova i picchi locali in un segnale basandosi sulla pendenza e sull'energia.

    Args:
        slope (np.ndarray): La pendenza del segnale.
        energy (np.ndarray): L'energia del segnale.

    Returns:
        np.ndarray: Un array contenente i picchi locali.
    """
    num_crit = len(energy)
...
def lpcoeff(speech_frame, model_order):
    """
    Calcola i coefficienti di predizione lineare (LPC) per un frame di segnale vocale.
    Utilizza l'algoritmo di Levinson-Durbin.

    Args:
        speech_frame (np.ndarray): Un frame del segnale vocale.
        model_order (int): L'ordine del modello di predizione lineare.

    Returns:
        tuple: Una tupla contenente i parametri LPC (lpparams) e le autocorrelazioni (R).
    """
    eps = np.finfo(np.float64).eps
    # ----------------------------------------------------------
    # (1) Calcola i ritardi di autocorrelazione
    # ----------------------------------------------------------
...
def llr(clean_speech, processed_speech, fs, used_for_composite=False, frameLen=0.03, overlap=0.75):
    """
    Calcola la misura Likelihood Ratio (LLR) tra due segnali vocali.

    Args:
        clean_speech (np.ndarray): Il segnale vocale pulito.
        processed_speech (np.ndarray): Il segnale vocale processato.
        fs (int): La frequenza di campionamento dei segnali.
        used_for_composite (bool, optional): Indica se la misura LLR viene utilizzata per il calcolo di una misura composita. Defaults to False.
        frameLen (float, optional): La lunghezza del frame in secondi. Defaults to 0.03.
        overlap (float, optional): La sovrapposizione tra frame adiacenti. Defaults to 0.75.
    ...
    skiprate    = int(np.floor((1 - overlap) * frameLen * fs)) # Salto della finestra in campioni
    if fs < 10000:
        P = 10 # Ordine dell'analisi LPC
    else:
        P = 16 # Questo potrebbe variare a seconda della frequenza di campionamento.
    ...
    if not used_for_composite:
        distortion[distortion > 2] = 2 # Questa linea non è nella misura composita ma nell'implementazione matlab di llr di loizou
...
def wss(clean_speech, processed_speech, fs, frameLen=0.03, overlap=0.75):
    """
    Calcola la misura Weighted Spectral Slope (WSS) tra due segnali vocali.
    ...
    Returns:
        float: La misura WSS.
    """    
    Kmax        = 20 # Valore suggerito da Klatt, pg 1280
    Klocmax     = 1 # Valore suggerito da Klatt, pg 1280
    alpha = 0.95
    if clean_speech.shape != processed_speech.shape:
        raise AudioMetricException('I segnali non hanno la stessa forma!')
    eps = np.finfo(np.float64).eps
    clean_speech = clean_speech.astype(np.float64) + eps
    processed_speech = processed_speech.astype(np.float64) + eps
    winlength   = round(frameLen * fs) # Lunghezza della finestra in campioni
    skiprate    = int(np.floor((1 - overlap) * frameLen * fs)) # Salto della finestra in campioni
    max_freq    = fs / 2 # Massima larghezza di banda
    num_crit    = 25# Numero di bande critiche
    ...
    bw_min = bandwidth[0]
    min_factor = np.exp (-30.0 / (2.0 * 2.303));#      % Punto a -30 dB del filtro
    ...
    num_frames = len(clean_speech) / skiprate - (winlength / skiprate)# Numero di frame
    start      = 1 # Campione iniziale
...
def pesq(clean_speech, processed_speech, fs, force_resample=True):
    """
    Calcola la misura Perceptual Evaluation of Speech Quality (PESQ) tra due segnali vocali.

    Args:
        clean_speech (np.ndarray): Il segnale vocale pulito.
        processed_speech (np.ndarray): Il segnale vocale processato.
        fs (int): La frequenza di campionamento dei segnali.
        force_resample (bool, optional): Forza il ricampionamento dei segnali a 16kHz se la frequenza di campionamento non è 8kHz o 16kHz. Defaults to True.

    Returns:
        tuple: Una tupla contenente il punteggio PESQ MOS e il punteggio MOS-LQO.
               Restituisce NaN per PESQ MOS se la frequenza di campionamento è 16000.

    Raises:
        ValueError: Se la frequenza di campionamento non è 8kHz o 16kHz e force_resample è False.
    """
    ...
    if fs == 8000:
        ...
        if mos_lqo > 4.5:
            mos_lqo = 4.5
        pesq_mos = np.NaN  # PESQ MOS non è definito per fs=16000 nella libreria originale
    else:
        raise ValueError('La frequenza di campionamento per PESQ deve essere 8 kHz o 16 kHz')
